...
class ShieldMedusa(BasicEquip):
    ...
    def __init__(self, x=1, y=1):
        name = (sttng.get_language())["items"]["shield_medusa"]["name"]
        sprite = ']'
        damage = 0
        critic = 0
        defense = 20
        ...
        super().__init__(name, sprite, x, y, damage, critic, defense, 5, "sm-]", func=self.use_function, to_player=to_player, battle_effect=self.use_alter_status, desc=(sttng.get_language())["items"]["shield_medusa"]["desc"], nonfunc=self.nonuse_function)

    def petrification(self, victim):
        victim.hp = 0
        hud.print_effect(f'\n\n[{victim.name}] {(sttng.get_language())["items"]["shield_medusa"]["effect"]}.')
