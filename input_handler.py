...
def valid_move(x, y, map):
    ...
    move_opt = ['w', 'a', 's', 'd']
    if x < 1  or y >= len(map[x - 1]): # To remove up when it's in the top.
        move_opt.remove('w')
    if len_y == x or y >= len(map[x + 1]): # To remove down when it's in the bottom.
        move_opt.remove('s')
    if y < 1: # To rigth up when it's in the right limit.
        move_opt.remove('a')
    if len_x == y: # To left up when it's in the left limit.
        move_opt.remove('d')
