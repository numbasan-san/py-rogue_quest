...
class Engine:
    ...
    def load_move(self, axis, move_vector, x, y, move): 
        sq = self.map[x][y]
        if not(isinstance(sq, (BasicEnemy))): # verifying if the next coor is an enemy
    ...
    def movement(self, axis, new_coor, move_vector):
        ...
        # Handle if the object/item is an instance of basic_item, BasicEquip, or BasicEnemy
        if isinstance(thing, (BasicItem, BasicEquip, BasicEnemy)):
            self.handle_item_or_enemy(thing, axis, new_player_coor, player_x, player_y)

        # Handle if the object is a BasicEnvironmentItem
        elif isinstance(thing, BasicEnvironmentItem):
    ...
    def load_entity(self, map_coors, start_entities, load_entity):
        for i in range(len(map_coors[load_entity + '_spawn_coords'])):
            while True:
                ...
                if (isinstance(entity, BasicEnemy) and entity.range <= self.dungeon_floor) or \
                isinstance(entity, (BasicItem, BasicEnvironmentItem)):
